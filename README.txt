...
- Cancelled, deleted, merged, rejected, and non-confirmed sessions are excluded from all aggregate hour totals.
- Upcoming sessions are determined using the session end date/time, not the date alone.
- Expired active sessions move to Past Sessions even before they are manually marked finished.
- Student Dashboard, Statistics, My Sessions, My Tutors, tutor/student cards, and relationship summaries use the same upcoming-session rule.
- Student payment pages no longer display internal role/permission explanations.
