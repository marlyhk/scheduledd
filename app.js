...
function courseUniRecord(course){
  const key=safe(course);
  return (DATA.courseUniversities||{})[key]||{};
}
function recordUniversitiesForCourse(course){
  const r=courseUniRecord(course);
  const raw=Array.isArray(r.universities)?r.universities:(r.university?[r.university]:[]);
  return uniqueSorted(raw);
}
function activeCourseUniversityPairs(source=DATA){
  const pairs=[];
  const activeCourses=new Map();
  const addActive=(course,university)=>{
    const c=prettyOptionLabel(course), u=prettyOptionLabel(university);
    if(!c)return;
    if(!activeCourses.has(optionKey(c)))activeCourses.set(optionKey(c),c);
    pairs.push({course:c,university:u});
  };
  // Active courses = courses currently assigned to active booking tutors.
  // We intentionally do NOT use old DATA.courses here, because removed courses can remain cached there.
  Object.values(source.users||{}).forEach(u=>{
    if(!u||u.role!=="tutor"||u.removed||u.hiddenFromBookings)return;
    (Array.isArray(u.courses)?u.courses:[]).forEach(c=>addActive(c,u.university||""));
  });
  // Admin Course ↔ University settings refine only active courses; they must not revive deleted courses.
  Object.entries(source.courseUniversities||{}).forEach(([key,r])=>{
    const course=prettyOptionLabel(r.name||key.replace(/_/g," "));
    if(!activeCourses.has(optionKey(course)))return;
    const unis=Array.isArray(r.universities)?r.universities:(r.university?[r.university]:[]);
    unis.forEach(u=>pairs.push({course,university:prettyOptionLabel(u)}));
  });
  return pairs;
}
function currentAvailableCourses(source=DATA){
  // IMPORTANT: do not read old DATA.courses here, because deleted courses can stay cached there.
  return uniqueSorted(activeCourseUniversityPairs(source).map(p=>p.course));
}
function currentAvailableUniversities(source=DATA){
  const names=[];
  Object.values(source.users||{}).forEach(u=>{if(u&&u.university)names.push(u.university)});
  Object.values(source.publicTutors||{}).forEach(t=>{if(t&&t.university)names.push(t.university)});
  activeCourseUniversityPairs(source).forEach(p=>{if(p.university)names.push(p.university)});
  return uniqueSorted(names);
}
function coursesForUniversity(university,source=DATA){
  const uni=optionKey(university);
  const pairs=activeCourseUniversityPairs(source);
  if(!uni)return currentAvailableCourses(source);
  return uniqueSorted(pairs.filter(p=>optionKey(p.university)===uni).map(p=>p.course));
}
function courseMatchesUniversity(course,university,source=DATA){
  if(!university)return true;
  const ckey=optionKey(course), ukey=optionKey(university);
  return activeCourseUniversityPairs(source).some(p=>optionKey(p.course)===ckey && optionKey(p.university)===ukey);
}
...
auth.onAuthStateChanged(async u=>{
  ...
  await loadData();
  if(typeof mergeExistingConsecutiveStudentBookings==="function"){
    try{const merged=await mergeExistingConsecutiveStudentBookings();if(merged)await loadData();}catch(e){console.warn("Consecutive-session migration skipped:",e);}
  }
...
function bookingIsFinanciallyExcluded(b){
  const status=String(b?.status||"").toLowerCase().trim();
  return !b||b.financialExcluded===true||b.deleted===true||!!b.cancelledAt||!!b.canceledAt||!!b.cancelledBy||!!b.canceledBy||status==="cancelled"||status==="canceled"||status==="deleted"||status==="rescheduled"||status==="merged";
}
function bookingCountsFinancially(b){return !bookingIsFinanciallyExcluded(b)}
function bookingPaymentEntries(b){
  if(!bookingCountsFinancially(b))return [];
...
function myBookings(){let b=list(DATA.bookings).filter(x=>!x.mergedInto&&String(x.status||"").toLowerCase()!=="merged");if(profile.role==="admin")return b;if(profile.role==="tutor")return b.filter(x=>x.tutorId===currentUser.uid);return b.filter(x=>x.studentId===currentUser.uid)}
...
function allAssignableCourses(university=""){
  return coursesForUniversity(university,DATA);
}
...
async function setBookingPaymentStatus(bookingId,shouldBePaid){
  ...
  if(!bookingCountsFinancially(b))return alert("Cancelled sessions are excluded from earnings and cannot be marked paid or unpaid.");
  if(profile.role!=="admin"&&!(profile.role==="tutor"&&b.tutorId===currentUser.uid))return alert("This action is not available from this account.");
...
function s92Tutors(){
  const base=(typeof tutors==="function"?tutors():s92List(DATA.users||{}).filter(u=>u.role==="tutor"));
  return base.filter(t=>!t.removed&&!t.hiddenFromBookings);
}
function s92Universities(){return currentAvailableUniversities(DATA)}
function s92TutorsForUniversity(){
  const uni=S92_BOOKING.university||"";
  if(!uni)return s92Tutors();
  return s92Tutors().filter(t=>optionKey(t.university)===optionKey(uni)||(t.courses||[]).some(c=>courseMatchesUniversity(c,uni,DATA)));
}
function s92Courses(){
  return currentAvailableCourses(DATA).map(name=>({name,code:name,id:safe(name)}));
}
function s92TutorCourses(tutor){
  const ids=Array.isArray(tutor?.courses)?tutor.courses:[];
  const uni=S92_BOOKING.university||"";
  const out=uniqueSorted(ids.map(prettyOptionLabel).filter(c=>!uni||courseMatchesUniversity(c,uni,DATA)||optionKey(tutor.university)===optionKey(uni)));
  return out.length?out:["General Tutoring"];
}
function s92SelectedTutor(){
  const list=s92TutorsForUniversity();
  if(!list.some(t=>t.id===S92_BOOKING.tutorId))S92_BOOKING.tutorId=(list[0]&&list[0].id)||"";
  return user(S92_BOOKING.tutorId)||list[0]||{};
}
function s92SelectedCourse(){
  const tutor=s92SelectedTutor();
  const list=s92TutorCourses(tutor);
  if(!list.some(c=>optionKey(c)===optionKey(S92_BOOKING.course)))S92_BOOKING.course=list[0]||"General Tutoring";
  return S92_BOOKING.course || list[0] || "General Tutoring";
}
...
function s92RenderBookingPanel(){
  const universities=s92Universities();
  const tutorList=s92TutorsForUniversity();
  ...
  return `<div class="s92-card">
    ...
    <label>University</label>
    <select onchange="S92_BOOKING.university=this.value;S92_BOOKING.tutorId='';S92_BOOKING.course='';S92_BOOKING.date='';S92_BOOKING.time='';s92RenderBookingPage();"><option value="">All universities</option>${universities.map(u=>`<option value="${safeOptionText(u)}" ${optionKey(S92_BOOKING.university)===optionKey(u)?"selected":""}>${safeOptionText(u)}</option>`).join("")}</select>
    <label>Tutor</label>
    ...
    <label>Course</label>
    <select onchange="S92_BOOKING.course=this.value;S92RenderSafe();">${courseList.map(c=>`<option value="${safeOptionText(c)}" ${optionKey(s92SelectedCourse())===optionKey(c)?"selected":""}>${safeOptionText(c)}</option>`).join("")}</select>
...
function S92RenderSafe(){S92_BOOKING.date='';S92_BOOKING.time='';s92RenderBookingPage();}
...
function scmHash(text){let h=2166136261;for(let i=0;i<text.length;i++){h^=text.charCodeAt(i);h=Math.imul(h,16777619);}return (h>>>0).toString(36);}
function scmCleanRecord(record){const out={...record};delete out.id;delete out.key;return out;}
function scmCombinedPayments(group){
...
function scmReceiptArchive(group,canonicalId,when){
  const map=new Map();
  group.forEach(b=>{
...
async function confirmBooking(){
  try{
    ...
    let finalBookingId=ref.key;
    try{finalBookingId=await mergeConsecutiveStudentBookingsForBooking(ref.key);}catch(e){console.warn('Session was booked, but automatic merge could not run:',e);}
    await loadData();
    if(typeof checkMilestonesAfterBooking==="function")await checkMilestonesAfterBooking();
    document.getElementById("content").innerHTML=s92Confirmation(finalBookingId||ref.key);
...
function s92StudentPaymentsPage(){
  ...
  document.getElementById("content").innerHTML=`<div class="s92-card"><h2>Payments</h2><div class="s92-payment-note">View your payment status here.</div>${rows.length?rows.map(b=>`<div class="s92-payment-card"><b>${b.course||"Session"}</b><br>${b.date||""} • ${s92FormatTime(b.start||b.time||"")}<br>Tutor: ${(user(b.tutorId)||{}).name||""}<br>Payment method: Whish<br>Total: ${s92Money(total(b))}<br>Status: ${s92PaymentBadge(b)}</div>`).join(""):s92Empty("💵","No payments yet","Your booked sessions will appear here.")}</div>`;
}
async function markPayment(id,paid){ const b=(DATA.bookings||{})[id]||{}; if(!s92CanEditPayment(b))return alert("This action is not available from this account."); return setBookingPaymentStatus(id,!!paid); }
async function togglePayment(id,index){
  const b=(DATA.bookings||{})[id]||{};
  if(!s92CanEditPayment(b))return alert("This action is not available from this account.");
...
function adminCourses(){
  $("content").innerHTML=`<div class="card"><h2>Course Management</h2><p class="muted">Courses shown around the website come from active tutor course assignments. Removed courses disappear from Request Access automatically.</p><table class="table"><tr><th>Tutor</th><th>University</th><th>Courses</th></tr>${tutors().map(t=>`<tr><td>${t.name}</td><td>${t.university||""}</td><td>${(t.courses||[]).join(", ")}</td></tr>`).join("")}</table><hr><div class="row"><select id="ct">${tutors().map(t=>`<option value="${t.id}">${t.name}</option>`)}</select><input id="cn" placeholder="Course name exactly: Physics 213"></div><button onclick="assignCourse()">Assign Course</button><button class="ghost" onclick="courseUniversitySettingsPage()">Course ↔ University Settings</button></div>`
}
async function assignCourse(){
  let t=user($("ct").value),c=prettyOptionLabel($("cn").value.trim());
  if(!c)return alert("Enter a course name.");
  const cs=mergeTextArrayCaseInsensitive(t.courses||[],[c]);
  await db.ref("users/"+$("ct").value+"/courses").set(cs);
  await db.ref("courses/"+safe(c)).set({name:c});
  if(t.university){
    const existing=recordUniversitiesForCourse(c);
    await db.ref("courseUniversities/"+safe(c)).set({name:c,universities:mergeTextArrayCaseInsensitive(existing,[t.university]),updatedAt:Date.now()});
  }
  await loadData();adminCourses();
}
function courseUniversitySettingsPage(){
  const courses=currentAvailableCourses();
  const universities=currentAvailableUniversities();
  $("content").innerHTML=`<div class="card"><h2>Course ↔ University Settings</h2><p class="muted">Choose which university each active course belongs to. Request Access and Booking will filter courses using this list. If no university is selected for a course, the app uses the tutor's university.</p>${courses.length?`<table class="table"><tr><th>Course</th><th>Assigned university/universities</th><th>Action</th></tr>${courses.map(c=>{const selected=recordUniversitiesForCourse(c);return `<tr><td>${c}</td><td>${selected.join(", ")||"Uses tutor university"}</td><td><button onclick="editCourseUniversities('${String(c).replace(/'/g,"\\'")}')">Edit</button></td></tr>`}).join("")}</table>`:`<p class="muted">No active courses yet.</p>`}<hr><h3>Available Universities</h3><p>${universities.join(", ")||"No universities yet."}</p></div>`;
}
function editCourseUniversities(course){
  const c=prettyOptionLabel(course);
  const universities=currentAvailableUniversities();
  const selected=recordUniversitiesForCourse(c).map(optionKey);
  const modal=document.createElement("div");
  modal.className="modal";
  modal.innerHTML=`<div class="modal-box"><h2>Assign University</h2><p><b>${c}</b></p><p class="muted">Select one or more universities where this course is available.</p><div class="checkbox-grid">${universities.length?universities.map(u=>`<label class="check"><input type="checkbox" class="courseUniCheck" value="${safeOptionText(u)}" ${selected.includes(optionKey(u))?"checked":""}>${safeOptionText(u)}</label>`).join(""):"<p class='muted'>No universities available yet.</p>"}</div><label>Add university if missing</label><input id="newCourseUniversity" placeholder="University name"><div class="row"><button onclick="saveCourseUniversities('${String(c).replace(/'/g,"\\'")}')">Save</button><button class="ghost" onclick="this.closest('.modal').remove()">Cancel</button></div></div>`;
  document.body.appendChild(modal);
}
async function saveCourseUniversities(course){
  const c=prettyOptionLabel(course);
  const chosen=[...document.querySelectorAll(".courseUniCheck:checked")].map(x=>x.value);
  const extra=prettyOptionLabel($("newCourseUniversity")?.value||"");
  const universities=mergeTextArrayCaseInsensitive(chosen,extra?[extra]:[]);
  await db.ref("courseUniversities/"+safe(c)).set({name:c,universities,updatedAt:Date.now()});
  document.querySelector(".modal")?.remove();
  await loadData();
  courseUniversitySettingsPage();
}
...
function bookWithTutor(id, course=""){
  preselectTutorId=id;
  const t=user(id)||{};
  S92_BOOKING.university=t.university||S92_BOOKING.university||"";
  S92_BOOKING.tutorId=id;
  S92_BOOKING.course=course||"";
  window.preselectCourse=course||"";
  openTab("Book");
}
function myStudentsPage(){
  const bookedIds=list(DATA.bookings).filter(b=>b.tutorId===currentUser.uid&&bookingCountsFinancially(b)).map(b=>b.studentId);
  ...
  $("content").innerHTML=`<div class="card"><h2>My Students</h2>${ss.length?`<div class="grid">${ss.map(s=>{
    ...
    const activeBs=bs.filter(bookingCountsFinancially);
    return `<div class="card"><h3>${s.name}</h3>
      ...
      <b>Upcoming</b><br>${activeBs.filter(b=>!b.done).map(b=>`${b.date} • ${b.course} • ${typeof formatTime12==="function"?formatTime12(b.start):b.start}`).join("<br>")||"<span class='muted'>None</span>"}
      <hr><b>Past</b><br>${activeBs.filter(b=>b.done).map(b=>`${b.date} • ${b.course} • ${typeof formatTime12==="function"?formatTime12(b.start):b.start}`).join("<br>")||"<span class='muted'>None</span>"}
      <hr><b>Unpaid:</b> ${money(unpaid(activeBs))}</div>`;
...
function tutorMonthlyStats(tutorId, month){
  const bs=tutorBookingsFor(tutorId).filter(bookingCountsFinancially).filter(b=>!month||bookingMonth(b)===month);
  return {bs,hours:totalHours(bs),earned:bs.reduce((s,b)=>s+total(b),0),paidAmount:paid(bs),unpaidAmount:unpaid(bs)};
}
function exportTutorBookingsCSV(tutorId=currentUser.uid, month=""){
  const t=user(tutorId);
  const bs=tutorBookingsFor(tutorId).filter(bookingCountsFinancially).filter(b=>!month||bookingMonth(b)===month).sort((a,b)=>(a.date||"").localeCompare(b.date||"")||(a.start||"").localeCompare(b.start||""));
...
function financialPage(){
  let b=myBookings().filter(bookingCountsFinancially),month=new Date().toISOString().slice(0,7),mb=b.filter(x=>(x.date||"").startsWith(month));
...
function exportCSV(){const rows=[["Date","Time","Course","Tutor","Student/Group","Duration","Location","Payment Method","Total","Payments"]];myBookings().filter(bookingCountsFinancially).forEach(b=>rows.push([b.date,b.start,b.course,user(b.tutorId).name||"",user(b.studentId).name||"",b.duration,b.location,b.paymentMethod,total(b),(b.payments||[]).map(p=>`${p.name}: ${money(p.amount)} ${p.paid?"Paid":"Unpaid"}`).join(" | ")]));const csv=rows.map(r=>r.map(x=>`"${String(x??"").replaceAll('"','""')}"`).join(",")).join("\n"),blob=new Blob([csv],{type:"text/csv"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="scheduled-export.csv";a.click();URL.revokeObjectURL(url)}
...
async function cancelTutorBooking(id){
  ...
  await db.ref("bookings/"+id).update({status:"cancelled",cancelledAt:Date.now(),cancelledBy:currentUser.uid,cancelReason:reason,financialExcluded:true,financialExcludedAt:Date.now(),financialExclusionReason:"cancelled"});
...
function bookingRows(bs,edit){
  return bs.length?`<table class="table"><tr><th>Date</th><th>Time</th><th>Course</th><th>Tutor</th><th>Student/Group</th><th>Status</th><th>Details</th><th>Payments</th><th>Notes</th><th>Actions</th></tr>${bs.map(b=>{
    ...
    const financialActive=bookingCountsFinancially(b);
    const paymentEntries=bookingPaymentEntries(b);
    const paymentButton=bookingFullyPaid(b)?"Mark Unpaid":"Mark Paid";
    const actions=edit?`${canTutorControl?`<button onclick="rescheduleTutorBooking('${b.id}')">Reschedule</button><button class="danger" onclick="cancelTutorBooking('${b.id}')">Cancel</button>`:""}<button onclick="editBooking('${b.id}')">Edit</button>${financialActive?`<button onclick="markBookingPayment('${b.id}')">${paymentButton}</button><button onclick="markDone('${b.id}')">Mark Done</button>`:""}<button class="danger" onclick="deleteBooking('${b.id}')">Delete</button>`:"";
    const paymentHtml=!financialActive?"<span class='muted'>Excluded from earnings — session cancelled</span>":paymentEntries.length?paymentEntries.map((p,i)=>`${p.name}: ${s97Money(p.amount)} ${badge(p.paid)} ${edit?`<button onclick="togglePayment('${b.id}',${i})">${p.paid?"Mark Unpaid":"Mark Paid"}</button>`:""}`).join("<br>"):"<span class='muted'>No payment amount</span>";
...
/* ===== v9.14 Organization & Card UI upgrade (additive, no core replacement) ===== */
function orgEsc(v){return String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));}
function orgArr(v){return Array.isArray(v)?v:[];}
function orgStatus(u){return u.status||"active";}
function orgBookingActive(t){return t.bookingActive!==false;}
function orgPublicActive(p){return p.publicActive!==false && !p.hidden;}
function orgActiveBookings(){return list(DATA.bookings||{}).filter(b=>!b.deleted&&!b.done&&!['cancelled','deleted'].includes(String(b.status||'').toLowerCase()));}
function orgStudentBookings(id){return list(DATA.bookings||{}).filter(b=>b.studentId===id&&!b.deleted);}
function orgTutorBookings(id){return list(DATA.bookings||{}).filter(b=>b.tutorId===id&&!b.deleted);}
function orgUpcoming(bs){const today=todayISO();return bs.filter(b=>!b.done&&!['cancelled','deleted'].includes(String(b.status||'').toLowerCase())&&String(b.date||'')>=today);}
function orgPaidState(bs){if(!bs.length)return 'none';return unpaid(bs)>0?'unpaid':'paid';}
function orgBadge(text,kind=''){return `<span class="org-badge ${kind}">${orgEsc(text)}</span>`;}
function orgInitials(name){return String(name||'?').trim().split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase();}
function orgTimeline(s){return orgArr(s.relationshipTimeline).slice().sort((a,b)=>(b.at||0)-(a.at||0));}
async function orgAddTimeline(studentId,type,text,meta={}){const s=user(studentId)||{};const timeline=orgTimeline(s);timeline.unshift({type,text,at:Date.now(),...meta});await db.ref(`users/${studentId}/relationshipTimeline`).set(timeline.slice(0,250));}
function orgAssignmentHistory(studentId){
  const s=user(studentId)||{}, stored=orgArr(s.assignmentHistory), map=new Map();
  stored.forEach(x=>map.set(`${x.tutorId||''}|${x.course||''}`,{...x}));
  orgStudentBookings(studentId).forEach(b=>{const k=`${b.tutorId||''}|${b.course||''}`;const cur=map.get(k)||{tutorId:b.tutorId,course:b.course,since:b.createdAt||Date.parse(b.date)||Date.now()};cur.since=Math.min(cur.since||Infinity,b.createdAt||Date.parse(b.date)||Date.now());cur.active=orgUpcoming(orgStudentBookings(studentId).filter(x=>x.tutorId===b.tutorId&&x.course===b.course)).length>0;map.set(k,cur)});
  return [...map.values()].sort((a,b)=>(b.since||0)-(a.since||0));
}
const _orgAutoAssign=typeof autoAssignStudentFromBooking==='function'?autoAssignStudentFromBooking:null;
if(_orgAutoAssign){autoAssignStudentFromBooking=async function(studentId,tutorId,course){await _orgAutoAssign(studentId,tutorId,course);const s=user(studentId)||{};const history=orgAssignmentHistory(studentId);const key=`${tutorId}|${course}`;if(!history.some(x=>`${x.tutorId}|${x.course}`===key))history.unshift({tutorId,course,since:Date.now(),active:true});else history.forEach(x=>{if(`${x.tutorId}|${x.course}`===key)x.active=true});await db.ref(`users/${studentId}/assignmentHistory`).set(history);await orgAddTimeline(studentId,'booking',`Booked ${course} with ${(user(tutorId)||{}).name||'tutor'}`,{tutorId,course});};}
const _orgTutorsForCourseAndUniversity=typeof tutorsForCourseAndUniversity==='function'?tutorsForCourseAndUniversity:null;
if(_orgTutorsForCourseAndUniversity){tutorsForCourseAndUniversity=function(course,university){return _orgTutorsForCourseAndUniversity(course,university).filter(orgBookingActive);};}
async function toggleTutorBookingStatus(id){
  const t=user(id);if(!t)return;const next=!orgBookingActive(t);
  await db.ref(`users/${id}/bookingActive`).set(next);
  const affected=students().filter(s=>assignedTutorIdsForStudent(s.id).includes(id)||orgStudentBookings(s.id).some(b=>b.tutorId===id));
  for(const s of affected){const ids=new Set(orgArr(s.unavailableTutorIds));next?ids.delete(id):ids.add(id);await db.ref(`users/${s.id}/unavailableTutorIds`).set([...ids]);await orgAddTimeline(s.id,next?'tutor_reactivated':'tutor_unavailable',`${t.name||'Tutor'} is now ${next?'available':'unavailable'} for booking`,{tutorId:id});}
  await loadData();adminTutors();
}
async function togglePublicTutorStatus(id){const p=(DATA.publicTutors||{})[id];if(!p)return;const next=!orgPublicActive(p);await db.ref(`publicTutors/${id}`).update({publicActive:next,hidden:!next,updatedAt:Date.now()});await loadData();publicTutorProfilesPage();}
function orgTutorCard(t){const bs=orgTutorBookings(t.id),studentsCount=new Set(bs.map(b=>b.studentId).concat(assignedStudentsForTutor(t.id).map(s=>s.id))).size,up=orgUpcoming(bs).length;return `<article class="org-profile-card"><div class="org-card-head"><div class="org-avatar">${orgInitials(t.name)}</div><div><h3>${orgEsc(t.name)}</h3><p>${orgEsc(t.email||'')}</p></div>${orgBadge(orgBookingActive(t)?'Active':'Inactive',orgBookingActive(t)?'ok':'off')}</div><div class="org-card-body"><div class="org-meta"><span>Universities<b>${orgEsc(orgArr(t.universities).length?orgArr(t.universities).join(', '):(t.university||'—'))}</b></span><span>Courses<b>${orgArr(t.courses).length}</b></span><span>Students<b>${studentsCount}</b></span><span>Upcoming<b>${up}</b></span></div><p class="org-course-line">${orgArr(t.courses).map(c=>orgBadge(c)).join(' ')||'<span class="muted">No courses</span>'}</p></div><div class="org-card-actions"><button onclick="editTutor('${t.id}')">Edit info</button><button class="ghost" onclick="editTutorPhoto('${t.id}')">Photo</button><button class="${orgBookingActive(t)?'danger':'ghost'}" onclick="toggleTutorBookingStatus('${t.id}')">${orgBookingActive(t)?'Set inactive':'Set active'}</button><button class="danger" onclick="deleteTutor('${t.id}')">Remove access</button></div></article>`;}
function adminTutors(){
  const ts=tutors();
  $('content').innerHTML=`<section class="org-page"><div class="org-page-title"><div><h2>Booking Tutors</h2><p>Booking status here affects booking only. Public profile visibility stays separate.</p></div></div><div class="org-toolbar"><input id="orgTutorSearch" placeholder="Search tutor, course, university" oninput="orgFilterTutorCards()"><select id="orgTutorStatus" onchange="orgFilterTutorCards()"><option value="">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select></div><div id="orgTutorGrid" class="org-card-grid">${ts.map(orgTutorCard).join('')||'<p class="muted">No tutors yet.</p>'}</div><details class="org-create-panel"><summary>Add booking tutor</summary><div class="row"><input id="tn" placeholder="Full name"><input id="te" type="email" placeholder="Email"><input id="tp" placeholder="Temporary password"><input id="tw" placeholder="WhatsApp"><input id="tr" type="number" placeholder="Hourly rate"><input id="tuiv" placeholder="University"></div><input id="tl" placeholder="General locations"><button onclick="createAccount('tutor')">Create / Link Booking Tutor</button></details></section>`;
}
function orgFilterTutorCards(){const q=($('orgTutorSearch')?.value||'').toLowerCase(),st=$('orgTutorStatus')?.value||'';document.querySelectorAll('#orgTutorGrid .org-profile-card').forEach((el,i)=>{const t=tutors()[i],okQ=!q||JSON.stringify(t).toLowerCase().includes(q),okS=!st||(st==='active'&&orgBookingActive(t))||(st==='inactive'&&!orgBookingActive(t));el.style.display=okQ&&okS?'':'none';});}
function orgStudentCard(s){const bs=orgStudentBookings(s.id),tids=[...new Set(assignedTutorIdsForStudent(s.id).concat(bs.map(b=>b.tutorId).filter(Boolean)))],courses=[...new Set(assignedCoursesForStudent(s.id).concat(bs.map(b=>b.course).filter(Boolean)))],up=orgUpcoming(bs).length;return `<article class="org-profile-card org-clickable" data-university="${orgEsc(s.university||'')}" data-courses="${orgEsc(courses.join('|'))}" data-tutors="${orgEsc(tids.join('|'))}" data-status="${orgEsc(orgStatus(s))}" data-payment="${orgPaidState(bs)}" onclick="showStudentAdminProfile('${s.id}')"><div class="org-card-head"><div class="org-avatar">${orgInitials(s.name)}</div><div><h3>${orgEsc(s.name)}</h3><p>${orgEsc(s.university||'No university')}</p></div>${orgBadge(orgStatus(s),orgStatus(s)==='active'?'ok':'off')}</div><div class="org-card-body"><div class="org-meta"><span>Tutors<b>${tids.length}</b></span><span>Courses<b>${courses.length}</b></span><span>Upcoming<b>${up}</b></span><span>Balance<b>${money(unpaid(bs))}</b></span></div><p class="org-course-line">${courses.slice(0,4).map(c=>orgBadge(c)).join(' ')||'<span class="muted">No courses</span>'}</p></div><div class="org-card-actions"><button onclick="event.stopPropagation();editStudent('${s.id}')">Edit</button><button class="ghost" onclick="event.stopPropagation();editStudentTutors('${s.id}')">Tutors</button><button class="ghost" onclick="event.stopPropagation();editStudentCourses('${s.id}')">Courses</button></div></article>`;}
function adminStudents(){
  const ss=students().sort((a,b)=>(a.name||'').localeCompare(b.name||''));
  const universities=currentAvailableUniversities();
  const courses=currentAvailableCourses();
  $('content').innerHTML=`<section class="org-page"><div class="org-page-title"><div><h2>Students</h2><p>Filter students by university or course. These options update automatically whenever a new university or course is added.</p></div></div><div class="org-toolbar org-select-toolbar"><label>University<select id="orgStudentUni" onchange="orgFilterStudentCards()"><option value="">All universities</option>${universities.map(u=>`<option value="${orgEsc(u)}">${orgEsc(u)}</option>`).join('')}</select></label><label>Course<select id="orgStudentCourse" onchange="orgFilterStudentCards()"><option value="">All courses</option>${courses.map(c=>`<option value="${orgEsc(c)}">${orgEsc(c)}</option>`).join('')}</select></label></div><div id="orgStudentGrid" class="org-card-grid">${ss.map(orgStudentCard).join('')||'<p class="muted">No students yet.</p>'}</div><details class="org-create-panel"><summary>Add student</summary><div class="row"><input id="sn" placeholder="Full name"><input id="se" type="email" placeholder="Email"><input id="sp" placeholder="Temporary password"><input id="sph" placeholder="Phone"><select id="su"><option value="">University</option>${universities.map(u=>`<option value="${orgEsc(u)}">${orgEsc(u)}</option>`).join('')}</select></div><button onclick="addStudent()">Add Student</button></details></section>`;
}
function orgFilterStudentCards(){
  const u=$('orgStudentUni')?.value||'',c=$('orgStudentCourse')?.value||'';
  document.querySelectorAll('#orgStudentGrid .org-profile-card').forEach(el=>{
    const studentCourses=(el.dataset.courses||'').split('|').filter(Boolean);
    const ok=(!u||el.dataset.university===u)&&(!c||studentCourses.includes(c));
    el.style.display=ok?'':'none';
  });
}
function showStudentAdminProfile(id){const s=user(id);if(!s)return;const bs=orgStudentBookings(id).sort((a,b)=>(b.date||'').localeCompare(a.date||'')),hist=orgAssignmentHistory(id),timeline=orgTimeline(s),tids=[...new Set(assignedTutorIdsForStudent(id).concat(bs.map(b=>b.tutorId).filter(Boolean)))],courses=[...new Set(assignedCoursesForStudent(id).concat(bs.map(b=>b.course).filter(Boolean)))];$('content').innerHTML=`<section class="org-page"><button class="ghost" onclick="adminStudents()">← Back to students</button><div class="org-detail-hero"><div class="org-avatar large">${orgInitials(s.name)}</div><div><h2>${orgEsc(s.name)}</h2><p>${orgEsc(s.email||'')} · ${orgEsc(s.phone||'')} · ${orgEsc(s.university||'')}</p>${orgBadge(orgStatus(s),orgStatus(s)==='active'?'ok':'off')}</div><div class="org-card-actions"><button onclick="editStudent('${id}')">Edit profile</button><button class="ghost" onclick="editStudentTutors('${id}')">Assign tutors</button><button class="ghost" onclick="editStudentCourses('${id}')">Assign courses</button></div></div><div class="kpi-grid"><div class="kpi-card"><div class="kpi-label">Tutors</div><div class="kpi-value">${tids.length}</div></div><div class="kpi-card"><div class="kpi-label">Courses</div><div class="kpi-value">${courses.length}</div></div><div class="kpi-card"><div class="kpi-label">Total Sessions</div><div class="kpi-value">${bs.length}</div></div><div class="kpi-card"><div class="kpi-label">Upcoming</div><div class="kpi-value">${orgUpcoming(bs).length}</div></div><div class="kpi-card"><div class="kpi-label">Balance</div><div class="kpi-value">${money(unpaid(bs))}</div></div></div><div class="org-detail-grid"><div class="card"><h3>Private admin notes</h3><textarea id="orgStudentNotes">${orgEsc(s.adminNotes||'')}</textarea><button onclick="saveStudentAdminNotes('${id}')">Save notes</button><h3>Status</h3><select id="orgStudentStatusEdit"><option ${orgStatus(s)==='active'?'selected':''}>active</option><option ${orgStatus(s)==='pending'?'selected':''}>pending</option><option ${orgStatus(s)==='inactive'?'selected':''}>inactive</option><option ${orgStatus(s)==='archived'?'selected':''}>archived</option></select><button onclick="saveStudentStatus('${id}')">Update status</button></div><div class="card"><h3>Assignment history</h3>${hist.map(h=>`<div class="timeline-item"><b>${orgEsc((user(h.tutorId)||{}).name||'Tutor')}</b> · ${orgEsc(h.course||'')}<br><span class="muted">Since ${new Date(h.since||Date.now()).toLocaleDateString()} · ${h.active?'Active':'Inactive'}</span></div>`).join('')||'<p class="muted">No history yet.</p>'}</div><div class="card"><h3>Relationship timeline</h3>${timeline.map(x=>`<div class="timeline-item"><b>${orgEsc(x.text||x.type||'Update')}</b><br><span class="muted">${new Date(x.at||Date.now()).toLocaleString()}</span></div>`).join('')||'<p class="muted">No timeline events yet.</p>'}</div><div class="card"><h3>Sessions</h3>${bs.slice(0,20).map(b=>`<div class="timeline-item"><b>${orgEsc(b.course||'')}</b> · ${orgEsc((user(b.tutorId)||{}).name||'')}<br>${orgEsc(b.date||'')} · ${orgEsc(formatTime12(b.start||''))} · ${orgEsc(b.status|| (b.done?'completed':'confirmed'))}</div>`).join('')||'<p class="muted">No sessions.</p>'}</div></div></section>`;}
async function saveStudentAdminNotes(id){await db.ref(`users/${id}/adminNotes`).set($('orgStudentNotes').value);await orgAddTimeline(id,'notes','Admin notes updated');await loadData();showStudentAdminProfile(id);}
async function saveStudentStatus(id){const status=$('orgStudentStatusEdit').value;await db.ref(`users/${id}/status`).set(status);await orgAddTimeline(id,'status',`Student status changed to ${status}`);await loadData();showStudentAdminProfile(id);}
function publicTutorProfilesPage(){const ps=list(DATA.publicTutors||{}).sort((a,b)=>(a.name||'').localeCompare(b.name||''));$('content').innerHTML=`<section class="org-page"><div class="org-page-title"><div><h2>Public Tutor Profiles</h2><p>This visibility is separate from booking tutor status.</p></div></div><div class="org-card-grid">${ps.map(p=>`<article class="org-profile-card"><div class="org-card-head"><img class="org-photo" src="${orgEsc(publicPhoto(p))}" onerror="this.src='scheduled-icon.jpeg'"><div><h3>${orgEsc(p.name)}</h3><p>${orgEsc(p.university||'')}</p></div>${orgBadge(orgPublicActive(p)?'Visible':'Hidden',orgPublicActive(p)?'ok':'off')}</div><div class="org-card-body"><p>${orgArr(p.courses).map(c=>orgBadge(c)).join(' ')}</p><p>${money(p.rate)}/h · ${p.linkedTutorId?orgEsc((user(p.linkedTutorId)||{}).name||'Linked'):'Not linked'}</p></div><div class="org-card-actions"><button onclick="editPublicTutorProfile('${p.id}')">Edit info</button><button class="ghost" onclick="editPublicTutorPhoto('${p.id}')">Photo</button><button class="${orgPublicActive(p)?'danger':'ghost'}" onclick="togglePublicTutorStatus('${p.id}')">${orgPublicActive(p)?'Hide profile':'Show profile'}</button><button class="danger" onclick="deletePublicTutorProfile('${p.id}')">Delete</button></div></article>`).join('')||'<p class="muted">No public profiles yet.</p>'}</div><details class="org-create-panel"><summary>Add public tutor profile</summary><div class="row"><input id="pname" placeholder="Tutor name"><input id="puniversity" placeholder="University"><input id="prate" type="number" placeholder="Hourly rate"><select id="plink"><option value="">No linked booking tutor</option>${tutors().map(t=>`<option value="${t.id}">${orgEsc(t.name)}</option>`).join('')}</select></div><input id="pcourses" placeholder="Courses, comma separated"><input id="plocations" placeholder="Locations, comma separated"><label>Profile picture</label><input id="pphotoFile" type="file" accept="image/*"><textarea id="pdesc" placeholder="Description / teaching style"></textarea><button onclick="addPublicTutorProfile()">Add Public Profile</button></details></section>`;}
function courseUniversitySettingsPage(){const courses=currentAvailableCourses(),universities=currentAvailableUniversities();$('content').innerHTML=`<section class="org-page"><div class="org-page-title"><div><h2>Universities</h2><p>University cards update automatically from tutor, course and request data.</p></div></div><div class="org-card-grid">${universities.map(u=>{const cs=courses.filter(c=>courseMatchesUniversity(c,u,DATA));const ts=tutors().filter(t=>optionKey(t.university)===optionKey(u)||orgArr(t.universities).some(x=>optionKey(x)===optionKey(u)));const ss=students().filter(s=>optionKey(s.university)===optionKey(u));return `<article class="org-profile-card"><div class="org-card-head"><div class="org-avatar">${orgInitials(u)}</div><div><h3>${orgEsc(u)}</h3><p>University</p></div></div><div class="org-meta"><span>Courses<b>${cs.length}</b></span><span>Tutors<b>${ts.length}</b></span><span>Students<b>${ss.length}</b></span></div><p>${cs.slice(0,6).map(c=>orgBadge(c)).join(' ')||'<span class="muted">No courses</span>'}</p></article>`}).join('')||'<p class="muted">No universities yet.</p>'}</div><div class="card"><h3>Course ↔ University settings</h3>${courses.map(c=>`<div class="org-setting-row"><div><b>${orgEsc(c)}</b><br><span class="muted">${orgEsc(recordUniversitiesForCourse(c).join(', ')||'Uses tutor university')}</span></div><button onclick="editCourseUniversities('${String(c).replace(/'/g,"\\'")}')">Edit</button></div>`).join('')}</div></section>`;}
function adminCourses(){
  const courses=currentAvailableCourses();
  const universities=currentAvailableUniversities();
  $('content').innerHTML=`<section class="org-page"><div class="org-page-title"><div><h2>Courses</h2><p>Filter course cards by university or course. The lists update automatically from your current website data.</p></div></div><div class="org-toolbar org-select-toolbar"><label>University<select id="orgCourseUniFilter" onchange="orgFilterCourseCards()"><option value="">All universities</option>${universities.map(u=>`<option value="${orgEsc(u)}">${orgEsc(u)}</option>`).join('')}</select></label><label>Course<select id="orgCourseNameFilter" onchange="orgFilterCourseCards()"><option value="">All courses</option>${courses.map(c=>`<option value="${orgEsc(c)}">${orgEsc(c)}</option>`).join('')}</select></label></div><div id="orgCourseGrid" class="org-card-grid">${courses.map(c=>{const courseUniversities=recordUniversitiesForCourse(c);const ts=tutorsForCourse(c),bs=list(DATA.bookings||{}).filter(b=>b.course===c&&!b.deleted),ss=new Set(bs.map(b=>b.studentId).concat(students().filter(s=>assignedCoursesForStudent(s.id).includes(c)).map(s=>s.id))),month=new Date().toISOString().slice(0,7);return `<article class="org-profile-card" data-course="${orgEsc(c)}" data-universities="${orgEsc(courseUniversities.join('|'))}"><div class="org-card-head"><div class="org-avatar">${orgInitials(c)}</div><div><h3>${orgEsc(c)}</h3><p>${orgEsc(courseUniversities.join(', ')||'Tutor university')}</p></div></div><div class="org-meta"><span>Tutors<b>${ts.length}</b></span><span>Students<b>${ss.size}</b></span><span>This month<b>${bs.filter(b=>String(b.date||'').startsWith(month)).length}</b></span></div><div class="org-card-actions"><button onclick="editCourseUniversities('${String(c).replace(/'/g,"\'")}')">Edit universities</button></div></article>`}).join('')||'<p class="muted">No courses yet.</p>'}</div><details class="org-create-panel"><summary>Assign a course</summary><div class="row"><select id="ct">${tutors().map(t=>`<option value="${t.id}">${orgEsc(t.name)}</option>`).join('')}</select><input id="cn" placeholder="Course name"></div><button onclick="assignCourse()">Assign Course</button></details></section>`;
}
function orgFilterCourseCards(){
  const u=$('orgCourseUniFilter')?.value||'',c=$('orgCourseNameFilter')?.value||'';
  document.querySelectorAll('#orgCourseGrid .org-profile-card').forEach(el=>{
    const courseUniversities=(el.dataset.universities||'').split('|').filter(Boolean);
    const ok=(!c||el.dataset.course===c)&&(!u||courseUniversities.includes(u));
    el.style.display=ok?'':'none';
  });
}
...
function finChartData(metric){
  ...
  return months.map(month=>{
    ...
    let value=0;
    if(metric==="hours")value=totalHours(bs);
...
function receiptSnapshot(b,number,existing={}){
  ...
  const rate=Number(b.ratePerHour||tutor.rate||0),duration=Number(b.duration||0),amount=Number(total(b)||rate*duration||0);
...
async function shareReceiptWhatsApp(bookingId){
  try{const {doc,b,r}=await buildReceiptPdf(bookingId);if(!receiptCanSend(b))throw new Error('This action is not available from this account.');const blob=doc.output('blob'),file=new File([blob],`Scheduled-Receipt-${r.number}.pdf`,{type:'application/pdf'});const msg=`Hello ${r.studentName}, here is your Scheduled payment receipt ${r.number} for ${r.course} on ${receiptDateLabel(r.date)}. Thank you for using Scheduled.`;
...
s92StudentPaymentsPage=function(){
  const rows=s92List(DATA.bookings||{}).filter(b=>b.studentId===currentUser.uid).sort((a,b)=>(b.date||'').localeCompare(a.date||''));
  document.getElementById('content').innerHTML=`<div class="s92-card"><h2>Payments</h2><div class="s92-payment-note">View your payment status and download available receipts.</div>${rows.length?rows.map(b=>`<div class="s92-payment-card"><b>${b.course||'Session'}</b><br>${b.date||''} • ${s92FormatTime(b.start||b.time||'')}<br>Tutor: ${(user(b.tutorId)||{}).name||''}<br>Payment method: ${b.paymentMethod||'Whish'}<br>Total: ${s92Money(total(b))}<br>Status: ${s92PaymentBadge(b)}${receiptButtons(b)}</div>`).join(''):s92Empty('💵','No payments yet','Your booked sessions will appear here.')}</div>`;
};
...
/* ===== Scheduled v9.14: confirmed-hours, live upcoming-session classification, and cleaner student payment copy ===== */
function scheduledBookingCountsTowardHours(b){
  if(!b || !bookingCountsFinancially(b))return false;
  const status=String(b.status||'').toLowerCase().trim();
  return !['pending','requested','rejected','declined'].includes(status);
}
function scheduledSessionEndTimestamp(b){
  if(!b?.date)return NaN;
  const time=String(b.start||b.time||'').trim();
  if(!time){
    const endOfDay=new Date(`${b.date}T23:59:59`);
    return endOfDay.getTime();
  }
  const start=new Date(`${b.date}T${time.length===5?time+':00':time}`);
  if(Number.isNaN(start.getTime()))return NaN;
  return start.getTime()+Number(b.duration||1)*60*60*1000;
}
function scheduledSessionIsUpcoming(b,now=Date.now()){
  if(!scheduledBookingCountsTowardHours(b)||b.done===true)return false;
  const end=scheduledSessionEndTimestamp(b);
  if(Number.isNaN(end))return String(b.date||'')>=todayISO();
  return end>now;
}
function scheduledSessionIsPast(b,now=Date.now()){
  if(!scheduledBookingCountsTowardHours(b))return false;
  if(b.done===true)return true;
  const end=scheduledSessionEndTimestamp(b);
  if(Number.isNaN(end))return String(b.date||'')<todayISO();
  return end<=now;
}
function scheduledSortUpcoming(a,b){
  return String(a.date||'').localeCompare(String(b.date||''))||String(a.start||a.time||'').localeCompare(String(b.start||b.time||''));
}
function scheduledSortPast(a,b){
  return String(b.date||'').localeCompare(String(a.date||''))||String(b.start||b.time||'').localeCompare(String(a.start||a.time||''));
}

/* All hour totals now ignore cancelled, deleted, merged, rejected, and otherwise non-confirmed sessions. */
totalHours=function(bs){return (bs||[]).filter(scheduledBookingCountsTowardHours).reduce((sum,b)=>sum+Number(b.duration||0),0);};

/* Upcoming means the session is active, not marked done, and its actual end time has not passed. */
upcomingBookingsForUser=function(){return myBookings().filter(b=>scheduledSessionIsUpcoming(b)).sort(scheduledSortUpcoming);};
orgUpcoming=function(bs){return (bs||[]).filter(b=>scheduledSessionIsUpcoming(b)).sort(scheduledSortUpcoming);};

studentDashboardPage=function(){
  const nb=nextBooking(),my=myBookings(),month=thisMonthBookings(my),upcoming=upcomingBookingsForUser();
  $('content').innerHTML=`<div class="dashboard-hero"><h2 class="student-welcome-line">${scheduledGreeting()}, ${profile.name||'student'} ✨</h2><p class="student-welcome-sub">${scheduledGreetingSub()}</p>${renderStudentInspoBanner()}<div class="quick-actions"><button onclick="openTab('Book')">Book New Session</button><button class="ghost" onclick="emergencySessionsPage()">⚡ Need Help Today?</button></div></div><div class="kpi-grid"><div class="kpi-card"><div class="kpi-label">Next Session</div><div class="kpi-value">${nb?`${nb.date}<br><span class="small">${formatTime12(nb.start||nb.time)}</span>`:'None'}</div></div><div class="kpi-card"><div class="kpi-label">Hours This Month</div><div class="kpi-value">${totalHours(month)}</div></div><div class="kpi-card"><div class="kpi-label">Upcoming Sessions</div><div class="kpi-value">${upcoming.length}</div></div><div class="kpi-card"><div class="kpi-label">My Tutors</div><div class="kpi-value">${studentTutors(currentUser.uid).length}</div></div></div><div class="grid"><div class="card"><h2>Upcoming Sessions</h2>${upcoming.slice(0,5).map(b=>`<div class="timeline-item"><b>${b.course||'Session'}</b><br>${b.date||''} • ${formatTime12(b.start||b.time)}<br>${user(b.tutorId).name||''}<br><a href="${calendarLinkForBooking(b)}" target="_blank">Add to Google Calendar</a></div>`).join('')||"<p class='muted'>No upcoming sessions.</p>"}</div><div class="card"><h2>Assigned Courses</h2><p>${assignedCourseBadges(currentUser.uid)}</p><hr><h2>Announcements</h2>${getAnnouncementsForRole().slice(0,3).map(a=>`<div class="announcement-card"><b>${a.title||''}</b><p>${a.message||''}</p></div>`).join('')||"<p class='muted'>No announcements yet.</p>"}</div></div>`;
};

statsPage=function(){
  const confirmed=myBookings().filter(scheduledBookingCountsTowardHours);
  if(profile.role==='student'){
    const upcoming=confirmed.filter(b=>scheduledSessionIsUpcoming(b));
    $('content').innerHTML=`<div class="card"><h2>My Statistics</h2><div class="kpi-grid"><div class="kpi-card"><div class="kpi-label">Total Hours Studied</div><div class="kpi-value">${totalHours(confirmed)}</div></div><div class="kpi-card"><div class="kpi-label">Sessions Completed</div><div class="kpi-value">${confirmed.filter(x=>x.done===true).length}</div></div><div class="kpi-card"><div class="kpi-label">Upcoming Sessions</div><div class="kpi-value">${upcoming.length}</div></div><div class="kpi-card"><div class="kpi-label">Number of Tutors</div><div class="kpi-value">${studentTutors(currentUser.uid).length}</div></div></div></div>`;
  }else{
    $('content').innerHTML=`<div class="card"><h2>Tutor Statistics</h2><div class="kpi-grid"><div class="kpi-card"><div class="kpi-label">Total Students</div><div class="kpi-value">${[...new Set(confirmed.map(x=>x.studentId))].length}</div></div><div class="kpi-card"><div class="kpi-label">Total Sessions</div><div class="kpi-value">${confirmed.length}</div></div><div class="kpi-card"><div class="kpi-label">Hours Taught</div><div class="kpi-value">${totalHours(confirmed)}</div></div><div class="kpi-card"><div class="kpi-label">Average Rating</div><div class="kpi-value">${avgRating(currentUser.uid)}</div></div></div></div>`;
  }
};

bookingsPage=function(edit){
  const bs=myBookings();
  const upcoming=bs.filter(b=>scheduledSessionIsUpcoming(b)).sort(scheduledSortUpcoming);
  const cancelled=bs.filter(b=>!scheduledBookingCountsTowardHours(b)).sort(scheduledSortPast);
  const past=bs.filter(b=>scheduledSessionIsPast(b)).sort(scheduledSortPast);
  $('content').innerHTML=`<div class="card"><h2>Upcoming Sessions</h2>${bookingRows(upcoming,edit&&profile.role!=='student')}</div>${cancelled.length?`<div class="card"><h2>Cancelled / Released Sessions</h2>${bookingRows(cancelled,edit&&profile.role!=='student')}</div>`:''}<div class="card"><h2>Past Sessions</h2>${bookingRows(past,edit&&profile.role!=='student')}</div>`;
};

myStudentsPage=function(){
  const bookedIds=list(DATA.bookings).filter(b=>b.tutorId===currentUser.uid&&scheduledBookingCountsTowardHours(b)).map(b=>b.studentId);
  const assignedIds=assignedStudentsForTutor(currentUser.uid).map(s=>s.id);
  const ids=[...new Set([...bookedIds,...assignedIds])];
  const ss=ids.map(id=>({id,...user(id)})).filter(s=>s.role==='student');
  $('content').innerHTML=`<div class="card"><h2>My Students</h2>${ss.length?`<div class="grid">${ss.map(s=>{
    const sessions=list(DATA.bookings).filter(b=>b.tutorId===currentUser.uid&&b.studentId===s.id&&scheduledBookingCountsTowardHours(b));
    const upcoming=sessions.filter(b=>scheduledSessionIsUpcoming(b)).sort(scheduledSortUpcoming);
    const past=sessions.filter(b=>scheduledSessionIsPast(b)).sort(scheduledSortPast);
    return `<div class="card"><h3>${s.name||''}</h3><p>${s.email||''}<br>${s.phone||''}</p><p class="assigned-list">${assignedTutorIdsForStudent(s.id).includes(currentUser.uid)?'Assigned student':'Booked student'}</p><b>Upcoming</b><br>${upcoming.map(b=>`${b.date} • ${b.course||''} • ${formatTime12(b.start||b.time)}`).join('<br>')||"<span class='muted'>None</span>"}<hr><b>Past</b><br>${past.map(b=>`${b.date} • ${b.course||''} • ${formatTime12(b.start||b.time)}`).join('<br>')||"<span class='muted'>None</span>"}<hr><b>Unpaid:</b> ${money(unpaid(sessions))}</div>`;
  }).join('')}</div>`:`<p class="muted">No students yet. Students appear here after booking with you or when admin assigns them to you.</p>`}</div>`;
};

/* Student payment copy stays concise; permissions remain enforced invisibly in the logic. */
s92StudentPaymentsPage=function(){
  const rows=s92List(DATA.bookings||{}).filter(b=>b.studentId===currentUser.uid&&!b.mergedInto&&String(b.status||'').toLowerCase()!=='merged').sort((a,b)=>(b.date||'').localeCompare(a.date||''));
  document.getElementById('content').innerHTML=`<div class="s92-card"><h2>Payments</h2><div class="s92-payment-note">View your payment status and download available receipts.</div>${rows.length?rows.map(b=>`<div class="s92-payment-card"><b>${b.course||'Session'}</b><br>${b.date||''} • ${s92FormatTime(b.start||b.time||'')}<br>Tutor: ${(user(b.tutorId)||{}).name||''}<br>Payment method: ${b.paymentMethod||'Whish'}<br>Total: ${s92Money(total(b))}<br>Status: ${s92PaymentBadge(b)}${receiptButtons(b)}</div>`).join(''):s92Empty('💵','No payments yet','Your booked sessions will appear here.')}</div>`;
};

/* Keep authorization private instead of explaining role permissions to student-facing accounts. */
const scheduledOriginalSetBookingPaymentStatus=setBookingPaymentStatus;
setBookingPaymentStatus=async function(bookingId,shouldBePaid){
  const b=(DATA.bookings||{})[bookingId];
  if(!b)return alert('Booking not found.');
  if(!bookingCountsFinancially(b))return alert('This session is excluded from payment totals.');
  if(profile.role!=='admin'&&!(profile.role==='tutor'&&b.tutorId===currentUser.uid))return alert('This action is not available from this account.');
  return scheduledOriginalSetBookingPaymentStatus(bookingId,shouldBePaid);
};

/* Refresh time-sensitive student views while they remain open. */
function scheduledRefreshStudentTimeViews(){
  if(!profile||profile.role!=='student'||!currentUser)return;
  const active=[...document.querySelectorAll('.tabs button.active')].map(b=>b.textContent.trim())[0]||'';
  if(active==='Dashboard')studentDashboardPage();
  else if(active==='My Sessions')bookingsPage(false);
  else if(active==='My Tutors')myTutorsPage();
  else if(active==='Statistics')statsPage();
}
if(!window.__scheduledUpcomingRefreshTimer){
  window.__scheduledUpcomingRefreshTimer=setInterval(scheduledRefreshStudentTimeViews,60000);
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')scheduledRefreshStudentTimeViews();});
}
