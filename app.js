...
function adminStudents(){
 const visible=profile.role==='admin'?students():students().filter(s=>studentTutors(s.id).some(t=>t.id===currentUser.uid)||assignedTutorIdsForStudent(s.id).includes(currentUser.uid));
 const unis=currentAvailableUniversities(),courses=currentAvailableCourses();
 $('content').innerHTML=`<section class="org-page"><div class="org-page-title"><div><h2>${profile.role==='admin'?'Students / Groups':'My Students / Groups'}</h2><p>Compact profiles with assignments, sessions, balances, notes and history.</p></div></div><div class="org-toolbar org-scroll-filters"><input id="orgStudentSearch" placeholder="Search students" oninput="orgFilterStudents()"><select id="orgStudentUni" onchange="orgFilterStudents()"><option value="">All universities</option>${unis.map(u=>`<option>${orgEsc(u)}</option>`).join('')}</select><select id="orgStudentCourse" onchange="orgFilterStudents()"><option value="">All courses</option>${courses.map(c=>`<option>${orgEsc(c)}</option>`).join('')}</select><select id="orgStudentTutor" onchange="orgFilterStudents()"><option value="">All tutors</option>${tutors().map(t=>`<option value="${t.id}">${orgEsc(t.name)}</option>`).join('')}</select><select id="orgStudentStatus" onchange="orgFilterStudents()"><option value="">All statuses</option><option>active</option><option>pending</option><option>inactive</option><option>archived</option></select><select id="orgStudentPayment" onchange="orgFilterStudents()"><option value="">All payments</option><option value="paid">Paid</option><option value="unpaid">Unpaid</option></select></div><div id="orgStudentGrid" class="org-card-grid">${visible.map(orgStudentCard).join('')||'<p class="muted">No accounts yet.</p>'}</div>${profile.role==='admin'?`<details class="org-create-panel"><summary>Add student or group</summary><div class="row"><input id="sn" placeholder="Name"><input id="se" type="email" placeholder="Email"><input id="sp" placeholder="Password"><input id="sphone" placeholder="Phone"><input id="suniversity" placeholder="University"><select id="stype"><option>individual</option><option>group</option></select></div><input id="smembers" placeholder="Group members comma separated"><label>Assign tutor(s)</label><div class="checkbox-grid">${tutorCheckboxes('assignedTutor')}</div><label>Assign course(s)</label><div class="checkbox-grid">${courseCheckboxes('assignedCourse')}</div><button onclick="createAccount('student')">Create Student/Group</button></details>`:''}</section>`;
}
function orgFilterStudents(){const q=($('orgStudentSearch')?.value||'').toLowerCase(),u=$('orgStudentUni')?.value||'',c=$('orgStudentCourse')?.value||'',t=$('orgStudentTutor')?.value||'',s=$('orgStudentStatus')?.value||'',p=$('orgStudentPayment')?.value||'';document.querySelectorAll('#orgStudentGrid .org-profile-card').forEach(el=>{const ok=(!q||el.innerText.toLowerCase().includes(q))&&(!u||el.dataset.university===u)&&(!c||(el.dataset.courses||'').split('|').includes(c))&&(!t||(el.dataset.tutors||'').split('|').includes(t))&&(!s||el.dataset.status===s)&&(!p||el.dataset.payment===p);el.style.display=ok?'':'none';});}
...
function adminCourses(){const courses=currentAvailableCourses();$('content').innerHTML=`<section class="org-page"><div class="org-page-title"><div><h2>Courses</h2><p>Live statistics from tutors, students and sessions.</p></div></div><div class="org-card-grid">${courses.map(c=>{const ts=tutorsForCourse(c),bs=list(DATA.bookings||{}).filter(b=>b.course===c&&!b.deleted),ss=new Set(bs.map(b=>b.studentId).concat(students().filter(s=>assignedCoursesForStudent(s.id).includes(c)).map(s=>s.id))),month=new Date().toISOString().slice(0,7);return `<article class="org-profile-card"><div class="org-card-head"><div class="org-avatar">${orgInitials(c)}</div><div><h3>${orgEsc(c)}</h3><p>${orgEsc(recordUniversitiesForCourse(c).join(', ')||'Tutor university')}</p></div></div><div class="org-meta"><span>Tutors<b>${ts.length}</b></span><span>Students<b>${ss.size}</b></span><span>This month<b>${bs.filter(b=>String(b.date||'').startsWith(month)).length}</b></span></div><div class="org-card-actions"><button onclick="editCourseUniversities('${String(c).replace(/'/g,"\\'")}')">Edit universities</button></div></article>`}).join('')||'<p class="muted">No courses yet.</p>'}</div><details class="org-create-panel"><summary>Assign a course</summary><div class="row"><select id="ct">${tutors().map(t=>`<option value="${t.id}">${orgEsc(t.name)}</option>`).join('')}</select><input id="cn" placeholder="Course name"></div><button onclick="assignCourse()">Assign Course</button></details></section>`;}
